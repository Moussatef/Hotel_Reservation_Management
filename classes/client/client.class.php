...
class client extends DB
{
    ...
    {
        $req = "SELECT * FROM  client INNER JOIN reservation on client.id_client = reservation.id_client LEFT JOIN enfant_reservation ON  reservation.id_reservation  = enfant_reservation.id_reservation  LEFT JOIN enfant on enfant_reservation.id_enfant = enfant.id_enfant  INNER JOIN chambre_res ON reservation.id_reservation = chambre_res.id_reservation INNER JOIN bien ON chambre_res.id_bien = bien.id_bien INNER JOIN bien_ ON bien.ID_Bien_info = bien_.ID_Bien where client.id_personne = ? ";
        $stmt = $this->connect()->prepare($req);
